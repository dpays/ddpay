...
import {Client, Asset, Transaction, signTransaction, PrivateKey} from './../src/index-node'
...
describe('database api', function() {
    ...
    it('getConfig', async function() {
        ...
        assert.equal(result['DPAY_CHAIN_ID'], client.options.chainId)
        serverConfig = result
        // also test some assumptions made throughout the code
        const conf = await liveClient.database.getConfig()
        assert.equal(conf['DPAY_CREATE_ACCOUNT_WITH_DPAY_MODIFIER'], 30)
        assert.equal(conf['DPAY_CREATE_ACCOUNT_DELEGATION_RATIO'], 5)
        assert.equal(conf['DPAY_100_PERCENT'], 10000)
        assert.equal(conf['DPAY_1_PERCENT'], 100)
    })
    ...
    it('getBlock', async function() {
        ...
        assert.equal('0000000000000000000000000000000000000000', result.previous)
        assert.equal(serverConfig['DPAY_INIT_PUBLIC_KEY_STR'], result.signing_key)
    })
    ...
    it('getDiscussions', async function() {
        const r1 = await liveClient.database.getDiscussions('comments', {
            ...
            start_permlink: 're-pal-re-almost-digital-ddpay-a-strongly-typed-steem-client-library-20170702t131034262z',
            tag: 'almost-digital',
            limit: 1,
        })
    ...
    it('getVestingDelegations', async function() {
        this.slow(5 * 1000)
        const [delegation] = await liveClient.database.getVestingDelegations('dpay', '', 1)
        assert.equal(delegation.delegator, 'dpay')
