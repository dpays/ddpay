...
export const TEST_NODE = process.env['TEST_NODE'] || 'https://d.dpays.io'
...
export async function createAccount(): Promise<{username: string, password: string}> {
    ...
    const username = `ddpay-${ randomString(9) }`
    const response = await fetch('https://dpaytest.network/create', {
        method: 'POST',
