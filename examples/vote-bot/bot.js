...
// setup the ddpay client, you can use other nodes, for example gtg's public node at https://d.dpays.io
const client = new ddpay.Client('https://api.dpays.io')

// deserialize the posting key (in wif format, same format as you find on the dSite.io interface)
const key = ddpay.PrivateKey.from(POSTING_KEY)
...
const stream = client.blockchain.getOperationsStream({mode: ddpay.BlockchainMode.Latest})
